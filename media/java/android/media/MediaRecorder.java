                                      ...
                                      MicrophoneDirection
{
    ...
    private String mOzoTuneFilePath;
    ...
    /**
     * Interface definition of a callback to be invoked to communicate some
     * events about the recording.
     * @hide
     */
    public interface OnEventListener
    {
        /**
         * Called to indicate a wind noise event during recording.
         *
         * @param mr the MediaRecorder the info pertains to
         * @param ts timestamp of the event occurred in milliseconds
         * @param status true if noticeable wind level has been detected, false otherwise
         * @param level wind level value
         */
        void onWindEvent(MediaRecorder mr, int ts, boolean status, int level);

        /**
         * Called to indicate a sound source localization event during recording.
         *
         * @param mr the MediaRecorder the info pertains to
         * @param azimuth source azimuth in degrees, zero in front and
         *                positive values to the left. Range -180..179
         * @param elevation source elevation in degrees, zero in front and
         *                  positive values up. Range -90..90
         * @param strength source strength. Range 0..100
         * @param id source ID
         */
        void onSourceLocalizationEvent(MediaRecorder mr, int azimuth, int elevation, int strength, int id);

        /**
         * Called to indicate a sound source sector event.
         *
         * @param mr the MediaRecorder the info pertains to
         * @param azimuth sector azimuth in degrees, zero in front and
         *                positive values to the left. Range -180..180
         * @param elevation source elevation in degrees, zero in front and
         *                  positive values up. Range -90..90
         * @param id source ID
         */
        void onSourceSectorAzimuthEvent(MediaRecorder mr, int azimuth, int elevation, int id);

        /**
         * Called to indicate a sound source sector event.
         *
         * @param mr the MediaRecorder the info pertains to
         * @param azimuth sector width in degrees
         * @param elevation sector height in degrees
         * @param id source ID
         */
        void onSourceSectorWidthEvent(MediaRecorder mr, int width, int height, int id);

        /**
         * Called to indicate a sound levels event. The level range is 0...32768.
         *
         * @param mr the MediaRecorder the info pertains to
         * @param levelL Left channel audio level
         * @param levelR Right channel audio level
         * @param id source ID
         */
        void onAudioLevelsEvent(MediaRecorder mr, int levelL, int levelR);
    }

    /**
     * Register a callback to be invoked when an event occurs while recording.
     *
     * @param listener the callback that will be run
     * @hide
     */
    ...
    private class EventHandler extends Handler
    {
        ...
        /* Events related to OZO Audio recording */
        private static final int MEDIA_RECORDER_OZOAUDIO_WNR_EVENT = 0x7F100001;
        private static final int MEDIA_RECORDER_OZOAUDIO_SSLOC_EVENT = 0x7F100002;
        private static final int MEDIA_RECORDER_OZOAUDIO_SSLOC_SECTORS1_EVENT = 0x7F100003;
        private static final int MEDIA_RECORDER_OZOAUDIO_SSLOC_SECTORS2_EVENT = 0x7F100004;
        private static final int MEDIA_RECORDER_OZOAUDIO_AUDIOLEVELS_EVENT = 0x7F100005;
        ...
        public void handleMessage(Message msg) {
            ...
            case MEDIA_RECORDER_OZOAUDIO_WNR_EVENT:
                if (mOnEventListener != null) {
                    int windLevel = (msg.arg2 >> 1) & 255;
                    boolean status = (msg.arg2 & 1) == 1 ? true : false;
                    mOnEventListener.onWindEvent(mMediaRecorder, msg.arg1, status, windLevel);
                }
                return;

            case MEDIA_RECORDER_OZOAUDIO_SSLOC_EVENT:
                if (mOnEventListener != null) {
                    int azimuth = ((msg.arg1 >> 15) & 511) - 180;
                    int elevation = ((msg.arg1 >> 7) & 255) - 90;
                    int strength = msg.arg1 & 127;
                    int id = msg.arg2;

                    mOnEventListener.onSourceLocalizationEvent(mMediaRecorder, azimuth, elevation,
                        strength, id);
                }
                return;

            case MEDIA_RECORDER_OZOAUDIO_SSLOC_SECTORS1_EVENT:
                if (mOnEventListener != null) {
                    int azimuth = (msg.arg1 & 511) - 180;
                    int elevation = ((msg.arg1 >> 16) & 511) - 90;
                    int id = msg.arg2;

                    mOnEventListener.onSourceSectorAzimuthEvent(mMediaRecorder, azimuth, elevation, id);
                }
                return;

            case MEDIA_RECORDER_OZOAUDIO_SSLOC_SECTORS2_EVENT:
                if (mOnEventListener != null) {
                    int width = msg.arg1 & 511;
                    int height = (msg.arg1 >> 16) & 511;
                    int id = msg.arg2;

                    mOnEventListener.onSourceSectorWidthEvent(mMediaRecorder, width, height, id);
                }
                return;

            case MEDIA_RECORDER_OZOAUDIO_AUDIOLEVELS_EVENT:
                if (mOnEventListener != null)
                    mOnEventListener.onAudioLevelsEvent(mMediaRecorder, msg.arg1, msg.arg2);
                return;
    ...
    /**
     * Parameter mappings for OZO Audio.
     *
     * @hide
     */
    public final static class OzoAudioParameters
    {
        private OzoAudioParameters() {}

        /**
         * Key for Ozo parameters.
         */
        public static final String GENERIC_PARAM = "vendor.ozoaudio.generic.value";
        ...
        public static final String WINDSCREEN_NOTIFICATION = "vendor.ozoaudio.wnr-notification.value";

        // Audio Framing notifications
        public static final String FRAMING_NOTIFICATION = "vendor.ozoaudio.ssloc-notification.value";

        // Audio levels notifications
        public static final String AUDIOLEVELS_NOTIFICATION = "vendor.ozoaudio.audiolevels-notification.value";
         ...
         * Zero focus gain.
         */
        public static final double ZERO_GAIN = 0.0;

        /**
         * Min focus gain.
         */
        public static final double MIN_GAIN = 1.0;

        /**
         * Low focus gain.
         */
        public static final double LOW_GAIN = 2.0;

        /**
         * Default focus gain.
         */
        public static final double DEFAULT_GAIN = 3.0;

        /**
         * High focus gain.
         */
        public static final double HIGH_GAIN = 4.0;

        /**
         * Maximum focus gain.
         */
        public static final double MAX_GAIN = 5.0;

        /**
         * Default focus azimuth.
         */
        public static final double DEFAULT_AZIMUTH = 0.0;

        /**
         * Default focus elevation.
         */
        public static final double DEFAULT_ELEVATION = 0.0;
    }

    /**
     * Set Ozo Audio encoding parameters. Call this before prepare().
     *
     * @hide
     */
    ...
    /**
     * Set channel mask when Ozo Audio used as effect. Call this before prepare().
     *
     * @hide
     */
    ...
    /**
     * Set initial audio focus parameters. Call this before prepare().
     *
     * @hide
     */
    public void setInitialFocusParameters(double gain, double azimuth, double elevation)
    {
        ...
        setOzoInitParameter(OzoAudioParameters.FEAT_FOCUSELEVATION, Double.toString(elevation));
    }

    /**
     * Set initial audio focus sector parameters. Call this before prepare().
     *
     * @hide
     */
    ...
    /**
     * Enable audio focus. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Disable audio focus. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set audio focus gain. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set audio focus azimuth. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set audio focus elevation. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set audio focus sector width. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set audio focus sector height. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Enable WNR notifications (i.e., current wind level). Call this before prepare().
     * The level notifications are then received via {@link OnEventListener.onWindEvent}.
     *
     * @hide
     */
    ...
    /**
     * Enable audio WNR. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Disable audio WNR. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set initial WNR mode. Call this before prepare().
     *
     * @hide
     */
    public void setInitialWindNoise(boolean wnr) {
        ...
        setOzoInitParameter(OzoAudioParameters.FEAT_WINDSCREEN, value);
    }

    /**
     * Enable audio levels notifications (i.e., L + R levels). Call this before prepare().
     * The level notifications are then received via {@link OnEventListener.onAudioLevelsEvent}.
     *
     * @hide
     */
    ...
    /**
     * Enable audio noise suppression. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Disable audio suppression. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set initial noise suppression mode. Call this before prepare().
     *
     * @hide
     */
    ...
    /**
     * Enable sound source localization events. Call this before prepare().
     * The localization notifications are then received via
     * {@link OnEventListener.onSourceLocalizationEvent}.
     *
     * @hide
     */
    ...
    /**
     * Enable custom processing mode. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Disable custom processing mode. Call this during capture.
     *
     * @hide
     */
    ...
    /**
     * Set initialization parameter for Ozo Audio encoder. Call this before prepare().
     *
    ...
    /**
     * Set runtime parameter for Ozo Audio encoder. Call this after prepare().
     *
     * @param key Parameter key.
     * @param value Parameter value.
     *
    ...
    /**
     * Set OZO Audio runtime parameters.
     *
     * @throws IllegalStateException
     * @hide
     */
    public native void setOzoAudioRuntimeParameter(String param)
            throws IllegalStateException;


    /**
     * Sets the path of the Ozo Audio Tune file to be produced. Call this after setOutputFile() but before prepare().
     *
     * @param path The pathname to use.
     * @hide
     */
    ...
    /**
     * Prepares the recorder to begin receiving Ozo Audio Tune data.
     *
     * @hide
     */
    ...
    /**
     * Assign Ozo Tune file descriptor to underlying native object.
     *
     * @throws IllegalStateException, IOException
     * @hide
     */
    private native void _setOzoTuneFileFD(FileDescriptor fd) throws IllegalStateException, IOException;
}
